...
//     const { title, description, githubUrl, liveDemoUrl, techStack, type } = req.body;
//     if (!title || !description || !techStack || !type || techStack.length === 0) {
...
//     if (project.user.toString() !== userId && project.user._id?.toString() !== userId) {
...
//     if (techStack) {
//       // If techStack comes as a string (FormData) parse it
...
//     if (!deletedProject)
//       return res.status(404).json({ message: "Project not found", status: "error" });
