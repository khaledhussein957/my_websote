...
export const getEducation = async (req: Request, res: Response) => {
    try {
        ...
        if (!educations) return res.status(404).json({ message: "No educations found", status: "false" });
        return res.status(200).json({ message: "Educations fetched successfully", status: "true", data: educations });
...
export const deleteEducation = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        if (!deletedEducation) return res.status(404).json({ message: "Education not found", status: "error" });
